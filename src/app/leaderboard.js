...
function rank(report) {
  ...
  for (const c of report?.coins ?? []) {
    ...
    const seat = wallets.get(key) ?? {
      wallet: key, generatedUsd: 0, keptUsd: 0, holdersUsd: 0, daoUsd: 0, graduated: 0, coins: [],
    }
    ...
    // What a sharing creator gave away. Still generated by their coins, so it counts
    // toward their rank; just not toward what they kept.
    seat.holdersUsd += Number(c.holdersUsd) || 0
...
function seat(s, place) {
  ...
  return `<li class="seat">
    ...
    <div>
      ...
      <div class="gen">${money.show(money.round(s.keptUsd) + money.round(s.holdersUsd) + money.round(s.daoUsd))}</div>
    </div>
...
function row(s, place) {
  ...
  return `<tr>
    ...
    <td class="num"><b>${money.show(money.round(s.keptUsd) + money.round(s.holdersUsd) + money.round(s.daoUsd))}</b></td>
    <td class="num">${money.show(s.keptUsd)}${s.holdersUsd ? `<span class="sub">+ ${money.show(s.holdersUsd)} to holders</span>` : ''}</td>
...
function paint(report) {
  ...
  view.innerHTML = `<h1>Leaderboard</h1>
    ...
    <p class="foot-note">Fees generated is what the creator, their holders where they chose to share, and the DAO have earned together —
